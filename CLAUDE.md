...
Inside the resolver, the strategy chain runs first, then `withCombination(result)` post-processes in five ordered rules. The MIN-property bypass (Rule 2) is FIRST because the MIN concept's RxNorm property is the most authoritative source we have for a dedicated combination L5 — when present, it overrides every other path.

1. **Not a combination** → return result unchanged.
2. **Combination + MIN ancestor has an L5 in its property API** (the Phase-2B path) → `KEEP` with that L5, override the strategy chain's codes. Example: RxCUI 1799213 (Epclusa) — MIN ancestor 1799211 ("sofosbuvir / velpatasvir") has `J05AP55` in its `/property.json?propName=ATC`. RxClass's classMembers does NOT expose J05AP55 (classMembers for J05AP returns SCDs whose SourceId is the RxCUI itself, never an L5); ATCPROD does NOT expose it; byId for J05AP55 returns `{}`. The MIN's RxNorm property is the only public surface that carries this code. The resolver consults it, and when it returns an L5 the result envelope is stamped with `minProvenance: { minRxcui, code }` so the UI shows a "Reached via the MIN concept's RxNorm property API" note on the kept card. Same path also catches Bactrim (MIN 10831 → J01EE01) and Glyxambi (MIN 1598392 → A10BD19) — those used to resolve via Pass-2 MIN-equality and now resolve a step earlier via Rule 2 (same answer, cleaner provenance).
3. **Combination, L4-only or NO_ATC** → escalate to `COMBINATION_NO_DEDICATED_CODE`. The L4 (when present) sits in `codes`, ingredient breakdown in `combinationIngredients`. Example: RxCUI 200284 (HCTZ + valsartan) — MIN 214626's property API is empty, no L5 reachable anywhere → C09DA + per-ingredient L5s.
4. **Combination, KEEP with L5 — but partial coverage** → escalate to `COMBINATION_NO_DEDICATED_CODE`. The `shouldEscalateToCombinationNoCode` discriminator decides: if every kept L5 also appears in some ingredient's property-API codes, then the kept L5 is just one ingredient's monotherapy code, not a real combination class. We replace `codes` with the L4 parent(s) of those kept L5s (class names fetched async) so the UI's `combinationClassCard` carries the L4 label. The ingredient L5 is still visible through `combinationIngredients`. After Rule 2 ships, Rule 4 fires only when the MIN ancestor also lacks an L5 in its property API — Advil PM is the canonical case (MIN 644895 → empty), so Strategy 1's M01AE01 (ibuprofen mono) escalates to L4 M01AE.
5. **Combination, KEEP with L5 — full coverage via Pass-2 MIN-equality** → stay `KEEP`, attach ingredients. In practice this path is dead code after Rule 2 ships (the MIN concepts that carry the L5 in classMembers also tend to carry it in property API), but it remains as a safety net for any edge case where Pass-2 succeeds but the MIN property doesn't.
